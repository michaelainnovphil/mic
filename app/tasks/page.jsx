...
function TasksContent() {
  ...
  const myTeam = TEAM_MAP[myEmail] || "Unassigned";
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="bg-white/90 dark:bg-gray-800/90 p-6 rounded-xl shadow-xl w-96 backdrop-blur">
            <h3 className="text-lg font-bold mb-4 text-blue-900 dark:text-blue-300">Assign Task</h3>
            {selectedTask && (
              <>
                <p className="mb-2">
                ...
                <div className="flex justify-end gap-3">
                  <button onClick={() => setModalOpen(false)} className="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded-lg">
                    Cancel
                  </button>
                  <button onClick={handleAssignToMe} className="px-4 py-2 bg-blue-900 text-white rounded-lg">
                    Assign to Me
                  </button>
                </div>
              </>
